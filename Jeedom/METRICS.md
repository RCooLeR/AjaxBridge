...
`voltage`, `powerWtH` і `currentMA` — наявні ключі старого шаблону/проксі. Від комплекту `2026.09.25.1` нові команди `currentMA` явно мають одиницю **mA**, а `powerWtH` — **Wh** та назву «Consommation». Формули перерахунку відсутні: сирі числа зберігаються, наприклад 960 mA та 78485 Wh. `powerWtH` є показом лічильника енергії, а не миттєвою потужністю W. Існуюче позначення `generic_type: POWER` збережене для сумісності з плагіном; AjaxBridge визначає енергію за явною одиницею Wh.

Ця зміна шаблону застосовується лише під час створення відсутніх команд. Наявні ID, назви, одиниці, формули й історія не змінюються. Зокрема власні формули `/ 1000` для A/kWh залишаються: для таких команд не можна просто поставити mA/Wh без перевірки формули. Шаблони Socket та PHP-парсер цією зміною не оновлюються.
